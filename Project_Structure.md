...
| `src/store/factory.py` | **Selector**: Resolves the active store from `WATCH_STORE` (firestore = Slice 3b). |
| `src/poll.py` | **Poll engine**: Quota-capped `poll_active_watches` + `PollSummary` (Slice 3a). |
| `bruno/cheapsawari/` | **API Validation**: Bruno collection — offers + watches + poll seams (gate: 67/67 assertions). |
...
> **Roadmap (rolling MVP):** Slice 1 — fare-fetch seam ✅ · Slice 2 — watches + persistence ✅ · Slice 3a — quota-capped poll engine ✅ · Slice 3b — cloud wiring (Firestore + Cloud Run + Cloud Scheduler, needs Firebase details) · Slice 4 — signal + single-channel alert · Slice 5 — minimal dashboard.
